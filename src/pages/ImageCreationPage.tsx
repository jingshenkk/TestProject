...
import { getSampleAssets } from '@/mocks/assets';
...
export default function ImageCreationPage() {
  ...
  const shotListQuery = useQuery({ queryKey: ['shot-list', project?.id], queryFn: () => fetchShotList(project!.id), enabled: Boolean(project && !isMockProject) });
  const imageQuery = useQuery({ queryKey: ['images', project?.id], queryFn: () => fetchProjectImages(project!.id), enabled: Boolean(project && !isMockProject) });
  const assets = useMemo(() => {
    if (isMockProject) return getSampleAssets(activeTab);
  ...
  }, [activeTab, imageQuery.data, isMockProject, shotListQuery.data]);
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-auto">
        <ProcessStepBar />
        {isMockProject && <p className="mb-4 rounded-xl border border-[var(--accent-primary)]/25 bg-[var(--accent-primary-bg)] px-4 py-3 text-sm text-[var(--text-secondary)]">LOCAL MOCK · 当前展示的是本地样例资产，生成入口不会写入后端资产台账。</p>}
